...
from train_flux.src.sd3.generate import generate
from train_flux.src.sd3.condition import Condition
import time
...
def sample(
    ...
    with open(os.path.join(root_dir, f"best_img_detailedscore.jsonl"), "a") as f:
        data = {
            ...
            "evaluation": outputs,
            "filenames_batch": imagetoupdate,
        }
        f.write(json.dumps(data) + "\n")
    #####################################################
    # generate reflections first at each round
    # breakpoint()
    conditionimg_forreflections = [best_img_path]
    ...
    if prompt_refiner_args and prompt_refiner_args.get("run_refinement", False):
        ...
        evaluations = [json.dumps(outputs[topk_idx[0]])]
        refined_prompt_inputs = verifier.prepare_refine_prompt_inputs(images=[best_img_path], evaluations=evaluations, original_prompt=[original_prompt], current_prompt=[updated_prompt], reflections=update_reflections)
    ...
    bestimg = Image.open(best_img_path)
    bestimg = bestimg.resize((config_cp["pipeline_args"]["condition_size"], config_cp["pipeline_args"]["condition_size"]))
    position_delta = np.array([0, -config_cp["pipeline_args"]["condition_size"] // 16])
    conditionimg = Condition(condition=bestimg, condition_type="cot", position_delta=position_delta)
    ...
    if reflection_args and reflection_args.get("run_reflection", False):
        if update_reflections:
            prompts = [updated_prompt + " [Reflexion]: " + update_reflections[0]] * num_samples
        else: 
    ...
    if search_round == 1:
        ...
        if mean_score_curround >= max_mean_score:
            for i in range(len(images_for_prompt)):
                images_for_prompt[i].save(os.path.join(sample_path, f"{i:05}.png"))
        else:
